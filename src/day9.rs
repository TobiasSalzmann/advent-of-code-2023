...
pub fn main() {
    ...
    advent.part2("sum of extrapolated values: {}", sum_extrapolated_first_values(&sequences));
}

fn sum_extrapolated_values(sequences: &Vec<Vec<i32>>) -> i32 {
    sequences.iter()
        .map(extrapolated_value)
        .sum()
}

fn sum_extrapolated_first_values(sequences: &Vec<Vec<i32>>) -> i32 {
    sequences.iter()
        .map(extrapolated_first_value)
        .sum()
}
...
fn derivatives(sequence: &Vec<i32>) -> Vec<Vec<i32>> {
    ...
    while derivatives.last().unwrap().iter().any(|x| *x != 0) {
        let next_derivative = derivatives.last().unwrap()
